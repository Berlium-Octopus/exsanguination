...
ClientEvents.tick(e => {
    if (!Client.paused && Client.player.age % 2) return
    if (!Client.player.potionEffects.isActive('exsanguination:unlisted')) return
...
JEIEvents.information(event => {
  ...
  event.addItem('spelunkers_charm:rock', ['Obtained from smashing stone and cobblestone mostly with a wooden knife.'])
...
ItemEvents.tooltip(event=>{
    event.addAdvanced('#spartanweaponry:lead_weapons', (item, advanced, text)=>{
           text.remove(1)
             text.remove(1)
    })
})
...
ItemEvents.tooltip(event=>{
    event.addAdvanced('#spartanweaponry:silver_weapons', (item, advanced, text)=>{
           text.remove(1)
             text.remove(1)
    })
})
