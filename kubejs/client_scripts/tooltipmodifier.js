    ...
    return new TooltipModifier(stacks)
        .addAdvancedFunction(3, () => {
            let missingList = () => {
                ...
                if (!mainhandPieces.some(piece => player.mainHandItem.getItem().id === piece)) missingPieces.push("Mainhand");
                // If anyone could fix this help
                //if (!curiosPieces.some(piece => player.mainHandItem.getItem().id === piece)) missingPieces.push("Curios");
                if (!curiosPieces.some(piece => player.curiosInventory.getEquippedCurios() === piece)) missingPieces.push("Curios");
                if (!bootsPieces.some(piece => player.feetArmorItem.getItem().id === piece)) missingPieces.push("Boots");
                if (!offhandPieces.some(piece => player.offHandItem.getItem().id === piece)) missingPieces.push("Offhand");
