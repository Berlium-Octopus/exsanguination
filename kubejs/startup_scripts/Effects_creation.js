let $target = Java.loadClass("net.liopyu.entityjs.util.ContextUtils$EntityDistanceToPlayerContext")
/*
ForgeEvents.onEvent("net.minecraftforge.event.entity.living.LivingHurtEvent", (event) => {
    ...
    if (!attacker) 
    if (!target) return;
    let distance = attacker.distanceTo(target);
    let reach = attacker.getEntityReach()
    // I want it if a mob is closer then the reach of the player 
    // The Mob takes less damage due to being close to the player
    // A Blind Spot i think
    // Like the Skada mod 
    // $target.distanceToClosestPlayer
    // if reach = 0.5
    // threshold = 0.9x^{0.65} Function
    let threshold = reach * 0.65
    // The mod uses attackInfo.minReach a item custom attribute 
    if (distance < threshold) {
    // If The Entity is in 80% of the weapon's range?????
    // Idk
        let newDamage = amount *  0.5
        event.setAmount(newDamage);
        attacker.tell("Haha Lower Damage :) " + newDamage + "Original Damage" + amount);
    }
})
*/
Item.of(':resonarium_axe', '{Damage:0}')
...
StartupEvents.registry('mob_effect', event => {
  event.create('exsanguination:escapee_feast') // Create the effect under "kubejs:custom_effect"
    .color(0xBE59E5) // Sets the color of the Effect's Particles.
})
...
StartupEvents.registry('mob_effect', event => {
  event.create('exsanguination:unlisted') // Create the effect under "kubejs:custom_effect"
    ...
    .harmful() 
        ...
        const { player, server } = entity
        if (!entity || entity.level.isClientSide() || !entity.isPlayer()) return;
        server.scheduleInTicks(0, () => {
        if (!entity.potionEffects.isActive('exsanguination:unlisted')) {
