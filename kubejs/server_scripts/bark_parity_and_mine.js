...
Campfire.forEach(camp => {
  BlockEvents.rightClicked(camp, event => {
    const { player, block, server, level, item} = event
    if (event.hand != 'MAIN_HAND') return
    if (event.player.mainHandItem.getId() !== "flimsytorches:unlit_flimsy_torch") return
    if (block.properties.lit === true) {
      player.swing()
      if (Math.random() > 0.95) {
        level.spawnParticles("minecraft:angry_villager", true, block.x, block.y, block.z, 0, 0, 0, 1, 0.4)
        block.set(block.id, { infinite: false, facing: block.properties.facing, runs_out: true, lit: false });
        server.runCommandSilent(`playsound minecraft:block.fire.extinguish block @a ${block.x} ${block.y} ${block.z} 1 1`)
        player.setSecondsOnFire(10)
        item.shrink(1)
      } else {
      level.spawnParticles('smoke', true, block.x, block.y, block.z, 0, 0, 0, 20, 0.4)
      server.runCommandSilent(`playsound minecraft:item.firecharge.use block @a ${block.x} ${block.y} ${block.z} 1 1`)
      player.give("flimsytorches:flimsy_torch")
      item.shrink(1)
      }
    }
    event.cancel()
