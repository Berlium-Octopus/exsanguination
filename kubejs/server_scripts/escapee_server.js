...
EntityEvents.hurt(event => {
  ...
  let heal = 2
  if (player.hasEffect("alexscaves:irradiated")) return
  player.setFoodLevel(foodLevel - amps)
  let dummy = event.entity.type == "dummmmmmy:target_dummy"
  if (dummy) {
    event.entity.kill()
  }
  event.server.runCommandSilent(`playsound lostcities:entity.escapee.eat block @a ${player.x} ${player.y} ${player.z} 2.3 1`)
  if (dummy) return
  if (Math.random() > 0.3) {
    player.heal(heal * amps)
    event.server.runCommandSilent(`playsound scalinghealth:heart_crystal_use block @a ${player.x} ${player.y} ${player.z} 0.19 0.7`)
  }
})
