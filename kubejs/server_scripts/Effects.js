  ...
  PlayerEvents.tick(event=>{
    ...
    player.level.getEntitiesWithin(aabb).forEach(ent => {
      ...
      if (ent.type != "rootoffear:wilted") return
        player.potionEffects.add("rootoffear:courage", 500, 0, false, true)
  })
  })
*/
// Vommiter's Script
EntityEvents.hurt(event => {
    ...
    if(!player) return; 
    if(!player.potionEffects.isActive('exsanguination:escapee_feast')) return;
    ...
    let amps = player.getEffect('exsanguination:escapee_feast').getAmplifier() + 3
    let heal = 1
    player.setFoodLevel(foodLevel - amps)
    if(Math.random()<0.60) player.heal(heal * amps)
})
...
// Idk If This works
      event.server.runCommand(`/execute as ${event.player.username} run rootoffear spawnday getrelative`)
      event.player.damageHeldItem("main_hand", 1)
    }
